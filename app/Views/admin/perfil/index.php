...
$avatarCurrent = \App\Core\Security::sanitizeString($user->imagem_perfil ?? '');
$avatarPreview = $avatarCurrent ?: $avatarFallback;
$avatarFilename = $avatarCurrent ? basename($avatarCurrent) : 'Nenhum arquivo';
?>
...
<section class="space-y-6">
    ...
    <article class="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white/90 dark:bg-slate-900 p-6 shadow-sm">
        <div class="flex flex-col items-center text-center gap-4">
            <div class="relative h-32 w-32">
                ...
                <div class="relative flex h-full w-full items-center justify-center overflow-hidden rounded-[32px] border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
                    <?php $avatarFallback = '/uploads/fallback-images/fallback-avatar.webp'; ?>
                    <img src="<?= \App\Core\Security::sanitizeString($user->imagem_perfil ?? $avatarFallback) ?>" alt="Avatar do usuário" class="absolute inset-0 h-full w-full object-cover">
                </div>
            </div>
            <div>
        ...
        <div class="mt-6 space-y-3 text-sm text-slate-500 dark:text-slate-300">
            <p class="flex items-center gap-2"><i data-lucide="shield-check" class="h-4 w-4 text-emerald-500"></i> Acesso nível administrador</p>
            <p class="flex items-center gap-2"><i data-lucide="clock-3" class="h-4 w-4 text-blue-500"></i> Atualizado em tempo real</p>
            <p class="flex items-center gap-2"><i data-lucide="activity" class="h-4 w-4 text-indigo-500"></i> Segurança reforçada com 2FA</p>
        </div>
    </article>

    <article class="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm">
        <form action="/admin/perfil" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" class="text-xs font-semibold text-rose-500 hover:text-rose-600" onclick="resetProfileImage()">Remover imagem</button>
                </div>
                <div class="flex flex-col gap-4 rounded-3xl border border-dashed border-slate-300 dark:border-slate-700 bg-slate-50/70 dark:bg-slate-800/70 p-4 md:flex-row md:items-center">
                    <div class="relative h-28 w-28 overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex items-center justify-center">
                        <img id="avatarPreviewImg" src="<?= $avatarPreview ?>" alt="Preview da imagem" class="absolute inset-0 h-full w-full object-cover">
                    </div>
                    <div class="flex-1 space-y-3">
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            <label for="imagem-perfil-input" class="inline-flex items-center gap-2 rounded-2xl px-4 py-2 text-xs font-semibold shadow-sm cursor-pointer"
                            ...
                            <span id="avatarFileName" class="text-xs text-slate-500 dark:text-slate-300 truncate max-w-[220px]">
                                <?= \App\Core\Security::sanitizeString($avatarFilename) ?>
                            </span>
                        </div>
                    </div>
                    <input type="file" name="imagem_perfil" accept="image/png, image/jpeg, image/webp, image/svg+xml" class="hidden" id="imagem-perfil-input" onchange="handleProfileImageChange(this)">
                    <input type="hidden" name="imagem_perfil_remove" id="imagem-perfil-remove" value="0">
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <button type="submit" class="inline-flex items-center gap-2 rounded-2xl px-5 py-3 text-sm font-semibold shadow-sm bg-blue-600 text-white hover:bg-blue-700">
...
const profileAvatarFallback = '<?= $avatarFallback ?>';

function resetProfileImage() {
    const fileInput = document.getElementById('imagem-perfil-input');
    const fileName = document.getElementById('avatarFileName');
    const preview = document.getElementById('avatarPreviewImg');
    const removeInput = document.getElementById('imagem-perfil-remove');
    if (fileInput) fileInput.value = '';
    if (fileName) fileName.innerText = 'Nenhum arquivo';
    if (preview) preview.src = profileAvatarFallback;
...
function handleProfileImageChange(input) {
    ...
    const preview = document.getElementById('avatarPreviewImg');
    const removeInput = document.getElementById('imagem-perfil-remove');
    if (fileName) fileName.innerText = file?.name ?? 'Nenhum arquivo';
    if (file && preview) {
    ...
    if (removeInput) {
        removeInput.value = file ? '0' : removeInput.value;
    }
}
</script>
